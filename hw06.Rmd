...
In the above quadratic regression function, we have some useful error messages. But what about making the error message possibly even predictive? Let's try to improve by suggesting a country name (like in Google search). For example, we want our error message to be: "There is no country named Cenada in the Gapminder dataframe, did you mean Canada?"
